...
function userlist:initialize()
  ...
  function userlist.bar:draw()
    if self.state == "shut" then
      if lobby.darkMode then lg.draw(img["playerslist_closed"], self.x, self.y)
      else lg.draw(img["playerslist_closed_light"], self.x, self.y) end
      return
    end
  ...
  function userlist:click(x, y, b)
    ...
    for username, user in pairs(list) do
      m = m + fontHeight
      if y > m + fontHeight and y < m + 2*fontHeight then
